...
function renderSearch(report){
  ...
  const rows=report.predictions||[],groups=new Map();for(const p of rows){const key=JSON.stringify([p.item_a,p.item_b,p.sale_scenarios]);if(!groups.has(key))groups.set(key,p);}
  ...
  if(rows.length){root.append(routeTable([...groups.values()],rows));root.append(el('p','Best quantity for each pair and selling method. Select a row for assumptions, fees and other quantities.','muted'));}
...
function routeTable(rows,rankRows=rows){
  ...
  for(const p of rows){const row=el('tr',null,'selectable'+(selectedPrediction===p.prediction_id?' selected':'')),cell=el('td'),button=el('button','#'+rank.get(p.prediction_id),'row-select');button.setAttribute('aria-label',`Review rank ${rank.get(p.prediction_id)}: ${p.quantity_a} ${p.item_a} to ${p.item_b}. ${scenarioLabel(p)}`);button.addEventListener('click',()=>reviewPaper(p.prediction_id));cell.append(button);row.append(cell);row.dataset.predictionId=p.prediction_id;
    const scenario=el('td',null,'scenario');scenario.append(el('div','Steam: '+saleLabel(p.sale_scenarios?.steam)),el('div','DM: '+saleLabel(p.sale_scenarios?.dmarket)));row.append(scenario);
